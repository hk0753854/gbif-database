...
species_summary_df = (
    silver_df_with_species_nm.groupBy(
        "scientificName",
...
(species_summary_df.write.format("delta").mode("overwrite").saveAsTable(gold_table))

print(f"Gold table updated: {gold_table}, " f"records={species_summary_df.count()}")
...
(country_summary_df.write.format("delta").mode("overwrite").saveAsTable(gold_table))

print(f"Gold table updated: {gold_table}, " f"records={country_summary_df.count()}")

# COMMAND ----------

# DBTITLE 1,Create Yearly Observation Summary
year_summary_df = (
    silver_df_with_species_nm.filter(F.col("event_year").isNotNull())
    .groupBy(
        "event_year",
        "query_scientific_name",
        "japanese_name",
...
(year_summary_df.write.format("delta").mode("overwrite").saveAsTable(gold_table))

print(f"Gold table updated: {gold_table}, " f"records={year_summary_df.count()}")
...
(basis_summary_df.write.format("delta").mode("overwrite").saveAsTable(gold_table))

print(f"Gold table updated: {gold_table}, " f"records={basis_summary_df.count()}")

# COMMAND ----------

# DBTITLE 1,Filter Valid Geographic Observations
geographic_observations_df = (
    silver_df_with_species_nm.filter(F.col("decimalLatitude").isNotNull())
    .filter(F.col("decimalLongitude").isNotNull())
    .filter((F.col("decimalLatitude") >= -90) & (F.col("decimalLatitude") <= 90))
    .filter((F.col("decimalLongitude") >= -180) & (F.col("decimalLongitude") <= 180))
    .select(
        "gbifID",
        "scientificName",
        "japanese_name",
...
geographic_summary_df = (
    geographic_observations_df.withColumn(
        "latitude_grid",
        F.round(
...
(geographic_summary_df.write.format("delta").mode("overwrite").saveAsTable(gold_table))

print(f"Gold table updated: {gold_table}, " f"records={geographic_summary_df.count()}")